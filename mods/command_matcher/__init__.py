...
from mods import plaintext_if_prefix, remove_all_xml, remove_first_prefix
...
def match_command(event: Event,
    ...
    def match_args_(args: list, match_args: list) -> bool:
        ...
        for item in match_args:
            if item in args:
                return True
        log.debug.error('用户输入的参数里 没有任何参数与预设匹配')
        return False
    ...
    def match_text_(text: str, match_text: str) -> bool:
        ...
        if match_text in text:
            return True
        log.debug('用户输入的文本里 没有任何文本与预设匹配')
        return False
    ...
    for item in command:
        ...
        if pure_msg.startswith(item + ' ') and not arg_less:
            ...
            log.debug('不匹配 [' + item + '] 与 [' + pure_msg + ']')
            continue
        log.debug('匹配到命令头了！！！！！')
        if match_args:
            log.debug('存在match_args')
            if match_args_(args, match_args):
                return MC(event, args, text)
            else:
                log.debug('虽然匹配了命令头，但是没有匹配到参数')
                if not match_text:  #`如果没有match_text，就不继续匹配了
                    continue

        if match_text:
            log.debug('存在match_text')
            if match_text_(text, match_text):
                return MC(event, args, text)
            else:
                log.debug('虽然匹配了命令头，但是没有匹配到文本')
