...
def soyorin(session):
    ...
    '''
    # 动态导入模块
    module = importlib.import_module("session_filter")
    # 访问模块中的 function_info_list
    ban_manager = module.ban_manager

    if session.message.content.strip() == 'ign':
        output_lines = []
        for i, ban_dict in enumerate(ban_manager.ALL_BAN_DICTS, start=0):
    ...
    if session.message.content.startswith('-ign'):
        ...
        ban_manager.delete_item(item_index)
