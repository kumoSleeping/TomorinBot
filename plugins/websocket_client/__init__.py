...
def on_message(ws: websocket.WebSocketApp, message: str):
    ...
    if data['op'] == 4:
        # print(data)
        print("[websocket] Satori驱动器连接成功！")
        for login_info in data['body']['logins']:
            name = login_info['user'].get('name', login_info['user']['id'])
            status = login_info['status']
            status_desc = "\033[32m运行中\033[0m" if status == 1 else "\033[31m下线\033[0m"
            print(f"[websocket] [logining_caller] [{name}] 上线 [{login_info['platform']}] 状态: {status_desc}")
...
def websocket_():
    ...
    if not connections:
        print("\033[31m[websocket] 未配置任何连接信息\033[0m")
        return
